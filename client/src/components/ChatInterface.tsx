...
import { Lightbulb, Send, Paperclip, Bot, RotateCcw, Download, Mail, Calculator, Upload, FileDown, Type } from 'lucide-react';
...
export default function ChatInterface({ document, showInputInline = true, onMessageToDocument }: ChatInterfaceProps) {
  ...
  // Function to download message as PDF with proper math notation
  const downloadMessageAsPDF = (content: string, timestamp: string) => {
    const printWindow = window.open('', '_blank');
    if (printWindow) {
      const cleanContent = removeMarkupSymbols(content);
      const processedContent = renderMathForPrint(cleanContent);
      ...
      printWindow.document.write(`
        ...
        <body>
          ...
          <div class="footer">
            Generated by DocMath AI - Mathematical notation preserved
          </div>
          <script>
  ...
  // Function to download message as Word document
  const downloadMessageAsWord = (content: string, timestamp: string) => {
    const cleanContent = removeMarkupSymbols(content);
    const wordContent = `
    ...
    a.href = url;
    a.download = `ai_response_${new Date(timestamp).getTime()}.docx`;
  ...
  // Function to download message as TXT
  const downloadMessageAsTXT = (content: string, timestamp: string) => {
    const cleanContent = removeMarkupSymbols(content);
    const txtContent = `AI Response - DocMath AI
Generated on: ${new Date(timestamp).toLocaleString()}
...
Generated by DocMath AI`;
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `ai_response_${new Date(timestamp).getTime()}.txt`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  // Function to email message to user
  const emailMessage = async (content: string, timestamp: string) => {
    ...
    try {
      const response = await fetch('/api/email/send-response', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          content: removeMarkupSymbols(content),
          userEmail,
          timestamp
        }),
      });
      ...
      toast({
        title: "Email Sent",
        description: `AI response has been sent to ${userEmail}`,
      });
    } catch (error) {
  ...
  // Enhanced math rendering for print
  const renderMathForPrint = (content: string): string => {
    ...
    // Enhanced math rendering with better fallbacks for print
    // Block math: $$...$$
    processedContent = processedContent.replace(/\$\$([^$]+)\$\$/g, (match, expr) => {
      const cleanExpr = expr.trim();
      return `<div class="math-block">${cleanExpr}</div>`;
    });
    
    // Inline math: $...$
    processedContent = processedContent.replace(/\$([^$\n]+)\$/g, (match, expr) => {
      const cleanExpr = expr.trim();
      return `<span class="math-inline">${cleanExpr}</span>`;
    });
                    ...
                    {msg.role === 'assistant' && (
                      <div className="flex items-center space-x-1">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => downloadMessageAsPDF(msg.content, msg.timestamp)}
                          className="text-xs text-gray-400 hover:text-gray-600 h-6 px-1"
                        >
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => downloadMessageAsWord(msg.content, msg.timestamp)}
                          className="text-xs text-gray-400 hover:text-gray-600 h-6 px-1"
                        >
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => downloadMessageAsTXT(msg.content, msg.timestamp)}
                          className="text-xs text-gray-400 hover:text-gray-600 h-6 px-1"
                        >
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => emailMessage(msg.content, msg.timestamp)}
                          className="text-xs text-gray-400 hover:text-gray-600 h-6 px-1"
                        >
